 ...
 * 1. `## 幻灯片 N`
 * 2. 表格（Markdown table）
 * 3. title / ctrTitle 占位符 → `### 标题`；没有占位符时，短首段提升为标题
...
async function parsePptxWithZip(buffer: Buffer): Promise<string> {
  ...
  for (let i = 0; i < slidePaths.length; i++) {
    ...
    let titleTexts = extractPlaceholderTexts(bodyXml, /ctrTitle|title/i);
    const bodyParagraphs = extractParagraphTexts(bodyXml);

    // 许多课件不用 title 占位符，此时把短首段当作标题，避免整页都是正文
    if (titleTexts.length === 0) {
      const fallback = bodyParagraphs[0]?.trim() ?? '';
      if (looksLikeSlideTitle(fallback)) {
        titleTexts = [fallback];
      }
    }
...
/** 无 title 占位符时，仅把短、单行的首段提升为标题 */
function looksLikeSlideTitle(text: string): boolean {
